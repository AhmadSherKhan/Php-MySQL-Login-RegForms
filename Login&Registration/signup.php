...
if($_SERVER["REQUEST_METHOD"]=="POST"){
    ...
    if(empty($nameErr) && empty($emailErr) && empty($passwordErr) && empty($confirm_passwordErr) && empty($uppercaseErr) && empty($lowercaseErr) && empty($numberErr) && empty($symbolsErr) && empty($lenErr)){
        ...
        if($con){
            ...
            if(mysqli_num_rows($result) > 0){
                ...
                $link = "<a href='http://localhost/Login&Registration/verify-email.php?key=".$_POST['email']."&token=".$token."'>Click and Verify Email</a>";
                
                $to = $_POST['email'];
                $subject = "Verify your email Addresss";
                $message = 'Click on this link to verify your email address '.$link.'';
                $headers = 'From: ahmdshrkhnn@gmail.com' . "\r\n" .
                'Reply-To: ahmdshrkhnn@gmail.com' . "\r\n" .
                'X-Mailer: PHP/' . phpversion();
                
                mail($to,$subject,$message,$headers);
                
                $sql = "INSERT INTO users(name,email,password,token) VALUES ('$name','$email','$hashedpassword','$token')";
                if(mysqli_query($con,$sql)){

                    $success = "SignUp successfully, We have sent you verification link";
                    $name = "";
                    $email = "";
                    $password = "";
                    $confirm_password = "";
